...
def split_train_test(train_size: int = 1000, samples: int = 3000) -> tuple:
...
def make_plots(
    ...
    Args:
        tree_model (DecisionTreeClassifier): the model used for the plots
        X (np.array): The inputs samples
        y (np.array): The output samples
    ...
    try:
        ...
        os.system(
            "dot -Tpng out.dot -o"
            + os.path.join(IMAGES_PATH, "tree_{}_samples_split.png".format(samples_split))
        )
